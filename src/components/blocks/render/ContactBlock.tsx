...
export function ContactBlock({ id, data }: { id: string; data: BlockDataOf<'contact'> }) {
  return (
    <a
      href={`/api/vcard/${id}`}
      className="group flex items-center justify-center gap-2 rounded-[var(--radius-card)] bg-[var(--surface)] p-4 text-[15px] font-semibold text-[var(--text)] transition-colors hover:bg-[var(--surface-hover)]"
    >
